...
import { readChars, hamming, parse8pixelRow, parseHiresByteFromPixelRow, countBits, imageCoordinatesToByteOffset, cellOffsets, pixelLuminance, byte2Pixels, distance } from './graphics.mjs'
import { palette, quantize, quantize2index } from './quantizer.mjs'
...
function char2Tile (char, color, backgroundColor) {
  return Array.from(char).map(byte => byte2Pixels(byte, color, backgroundColor))
}
// which
function colorDistance (tile, char, color, backgroundColor) {
  const charColor = averageColorOfTile(char2Tile(char, color, backgroundColor))
  const tileColor = averageColorOfTile(tile)
...
function bestMatch (tile, chars, backgroundColor) {
  return supportedChars
    .map(i => [i, hammingDistance(tile, chars[i], palette[backgroundColor])])
...
function bestColorMatch (tile, chars, backgroundColor) {
  // const charIndex = 0
  let allDistances = []
  supportedChars.forEach(charIndex => {
    const distances = palette.map((color, colorIndex) => [colorIndex, charIndex,
      colorDistance(tile, chars[charIndex], color, backgroundColor) + hammingDistance(tile, chars[charIndex], backgroundColor)])
    allDistances = [...allDistances, ...distances]
  })
  const charAndColor = allDistances.reduce((a, v) => v[2] < a[2] ? v : a, [0, 0, Number.MAX_VALUE])
  // const charAndColor = supportedChars
  // .flatMap(charIndex =>
  //     palette
  //     .flatMap((color, colorIndex) => [colorIndex, charIndex,
  //       colorDistance(tile, chars[charIndex], color, backgroundColor)]
  //     )
  // )
  // .reduce((a, v) => v[2] < a[2] ? v : a, [0, 0, Number.MAX_VALUE])
  return { char: charAndColor[1], color: charAndColor[0] }
}
...
async function convertFile (filename, charSet) {
  ...
  // color match
  // const screenCodesAndColors = tiles.map(
  //   (t, i) => {
  //     console.log(i)
  //     return bestColorMatch(t, charSet, palette[backgroundColor])
  //   })
  // const screenCodes = screenCodesAndColors.map(c => c.char)
  // const colors = screenCodesAndColors.map(c => c.color)
